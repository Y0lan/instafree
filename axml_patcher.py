...
class AXMLPatcher:
    ...
    def _str(self, idx):
        if isinstance(idx, int) and 0 <= idx < len(self.strings):
            return self.strings[idx]
        return None

    def _iter_start_tags(self):
        """Yield (tag_name_idx, attrs) for every START_TAG.

        attrs items are (ns, name, raw, type, data).
        """
        for etype, _ehdr, edata in self.events:
            if etype != CHUNK_START_TAG or len(edata) < 28:
                continue
            (_line, _comment, _ns, name, attr_start, _attr_size,
             attr_count, _id_idx, _class_idx, _style_idx) = struct.unpack_from(
                '<IIIIHHHHHH', edata)
            # attr_start is relative to the ns field (offset 8).
            attr_off = 8 + attr_start
            if attr_off + attr_count * 20 > len(edata):
                attr_off = 28
            attrs = []
            for i in range(attr_count):
                ...
                if off + 20 > len(edata):
                    break
                a_ns, a_name, a_raw, _a_size, _a_res0, a_type, a_data = (
                    struct.unpack_from('<IIIHBBI', edata, off))
                attrs.append((a_ns, a_name, a_raw, a_type, a_data))
            yield name, attrs

    def get_application_name(self):
        """Return android:name of the <application> tag, or None."""
        try:
            app_idx = self.strings.index('application')
        except ValueError:
            return None
        name_attr = self._find_attr_string_idx(ATTR_NAME)
        for name, attrs in self._iter_start_tags():
            if name != app_idx:
                continue
            for _a_ns, a_name, _a_raw, a_type, a_data in attrs:
                if a_type != TYPE_STRING:
                    continue
                if name_attr is not None and a_name == name_attr:
                    return self._str(a_data)
                if self._str(a_name) == 'name':
                    return self._str(a_data)
        return None

    def get_package_name(self):
        """Return the package attribute of the <manifest> tag, or None."""
        try:
            man_idx = self.strings.index('manifest')
        except ValueError:
            return None
        for name, attrs in self._iter_start_tags():
            if name != man_idx:
